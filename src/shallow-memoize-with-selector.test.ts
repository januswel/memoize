import shallowMemoizeWithSelector from './shallow-memoize-with-selector'
...
describe('memoizeObject', () => {
  it('returns memoized function', () => {
    const filteredTodos = shallowMemoizeWithSelector(filter, selector)
    ...
    expect(filteredTodos(store3)).not.toBe(newers)
  })

  it('always calculates with no selectors', () => {
    const filteredTodos = shallowMemoizeWithSelector(filter, () => [])
